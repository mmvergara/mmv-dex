...
export const getUserProfileByEmail = async (supabase:SupabaseClient<DatabaseTypes>, email: string) => {
  return await supabase.from("profiles").select("*").eq("email", email).maybeSingle();
};

// Check if the user is an admin
export const checkIfUserIsAdminById = async (supabase: SupabaseClient<DatabaseTypes>, userId?: string) => {
